...
<details>
    ...
    <ol>
        ...
        <li>
            ...
            <ul>
                <li>시/도 → 시/군/구 필터링과 주소 기반 검색, 카카오맵과 연동기능, 카카오 길찾기 기능등을 결합하여 사용자의 실제 생활 반경을 기준으로 <strong>가장 가까운 충전소와 정비소를 정확하게 탐색하고, 필요시 해당 장소까지 안전하게 이동할 수 있도록 안내</strong>합니다.</li>
            </ul>
        </li>
        <li>
...
- 유헌상
  - 이번 프로젝트를 통해 팀장으로써 팀원분들이 가진 능력을 최대한으로 이끌어낼 수 있는 리더십이 중요하다는 것과 크롤링과 api를 통해서 가져온 데이터들을 어떻게 나누고 연결해서 사용하는지에 따라서 프로젝트 개발물의 퀄리티가 하늘과 땅차이로도 달라질 수 있다는 것을 배웠습니다.
    또, 프로젝트 초반 기획 단계에서 어떤 데이터를 어떻게 사용할지에 대한 구체적인 설계가 부족하다보니, 프로젝트를 진행하면서 새로 추가된 데이터도 있고 사용하지 않고 누락된 데이터도 존재하게 됐다. 시간이 더 충분했다면, 새로운 기능을 추가한다거나, 기존의 코드를 좀 더 세분화하여 모듈화작업을 해보았을텐데 그러지 못한 것이 아쉽다. 프로젝트를 진행하면서 공공데이터포털의 API 연동과 Dbeaver를 이용한 database 경험을 하였는데, 새로 배운 크롤링 은 직접 해보지 않은 것이 아쉽고, 조금 더 공부가 필요할 것 같다. 카카오맵 API를 사용하여 내 위치와 특정 위치를 표시해줄 때, 추가적인 정보를 직접 제공하고 싶었는데, 카카오맵에 관련 API가 없고, 하려면 직접 구현해야 했기 때문에 시도하지 못했다. 만약 다음에도 카카오맵을 이용하게 된다면, 현재 내 위치와 특정 장소의 거리를 구하고, 길찾기를 제공하는 방법 등에 고민이 필요할 듯하다.
- 김다빈
